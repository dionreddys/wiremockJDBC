...
/**
 *
 * Kesalahan ekstensi Wiremock
 *
 */
public class WireMockPostgres extends ResponseTransformer {

	private final DbManager manager;
	private final JsonConverter jsonConverter;
	private final ConfigHandler config;

	public WireMockPostgres() throws WireMockPostgresException {
		try {
		...
		} catch (final WireMockPostgresException e) {
			WireMockConfiguration.wireMockConfig().notifier().error(e.getMessage(), e);
			throw new WireMockPostgresException("Terjadi kesalahan saat menginisialisasi ekstensi DB.");
		}
	}
	...
	private QueryResults executeQueries(final RequestConfigHandler requestConfig, final Map<String, Object> queries)
		...
		if (queries.get("query") != null && queries.get("conditionQuery") != null) {
			throw new WireMockPostgresException("'query' dan 'conditionQuery' tidak dapat digunakan keduanya.");
		}
		final QueryResults result;
		if (queries.get("conditionQuery") != null) {
			String query = this.getQuery(queries.get("conditionQuery"));
			query = WireMockPostgresUtils.replaceQueryVariables(query, requestConfig);
			@SuppressWarnings("unchecked")
			...
			//Tidak ada kueri utama: kami mensimulasikan hasil dari 1 baris tanpa kolom, untuk memungkinkan eksekusi subkueri
			// ekstraksi massal.
	...
	/**
	 * Kueri secara opsional dapat dalam bentuk array, dalam hal ini kami menempelkannya.
	 */
	@SuppressWarnings("unchecked")
	...
	/**
	 * Jalankan dan parsing semua sub queri lalu masukkan hasilnya ke dalam hasil induk.
	 */
    ...
    private String getCsvDbPath() {
        final String filesRoot = System.getProperty("csv-root-dir", Main.filesRoot() == null ? "." : Main.filesRoot());
